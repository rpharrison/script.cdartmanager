...
def user_updates(details, type_):
    ...
    if type_ == "album":
        ...
        try:
            ...
            """, (details["local_id"], cu.get_str(details["path"])))
            db_details = c.fetchall()
            print (db_details)
            if db_details:
                ...
                """, (cu.get_str(details["artist"]), cu.get_str(details["title"]),
                ...
                c.execute("""\
                    ...
                    values (?, ?, ?, ?, ?, ?)
                """, (details["local_id"], cu.get_str(details["title"]), cu.get_str(details["artist"]),
                      cu.get_str(details["path"]), details["musicbrainz_albumid"], details["musicbrainz_artistid"]))
        except SQLError:
        ...
        try:
            ...
            """, (cu.get_str(details["artist"]), cu.get_str(details["title"]), details["musicbrainz_albumid"],
                  details["musicbrainz_artistid"], details["local_id"], cu.get_str(details["path"])))
        except SQLError:
            log("Error updating album table", xbmc.LOGERROR)
            traceback.print_exc()
    conn.commit()
...
def get_xbmc_database_info(background=False):
    ...
    dialog_msg("create", heading=__lng__(32021), message=__lng__(32105), background=background)
                    ...
                    try:
                        ...
                        if path == previous_path:
                            ...
                            if xbmcvfs.exists(os.path.join(path, '')):
                                ...
                                title = detail['title']
                                album_artist["artist"] = cu.get_str(
                                    artist_list_to_string(album['artist']).split(" / ")[0])
                                album_artist["path"] = cu.get_str(path)
                                album_artist[ArtType.CDART] = xbmcvfs.exists(
                                    sanitize(os.path.join(path, FileName.CDART)))
                                album_artist[ArtType.COVER] = xbmcvfs.exists(
                                    sanitize(os.path.join(path, FileName.FOLDER)))
                                previous_path = path
                                    ...
                                    if len(title_match.groups()) > 1:
                                        if title_match.group(2):
                                            ...
                                            if path_match:
                                                ...
                                                album_artist["disc"] = 1
                                            album_artist["title"] = cu.get_str((title.replace(" -", "")).rstrip())
                                    else:
                                        if path_match:
                                            if len(path_match.groups()) > 0:
                                                if path_match.group(1):
...
def get_album_cdart(album_path):
    ...
    return bool(sanitize(xbmcvfs.exists(os.path.join(album_path, FileName.CDART))))


def get_album_coverart(album_path):
    log("Retrieving cover art status")
    return bool(xbmcvfs.exists(sanitize(os.path.join(album_path, FileName.FOLDER))))
...
def store_alblist(local_album_list, background=False):
    ...
    try:
        for album in local_album_list:
            ...
            log("Album Path: %s" % sanitize(album["path"]))
            ...
            try:
                ...
                """, (album["local_id"], cu.get_str(album["title"]), cu.get_str(album["artist"]),
                      cu.get_str(sanitize(album["path"])), ("False", "True")[album["cdart"]],
    ...
    for artist in local_artist_list:
        try:
            try:
                ...
                """, (artist["local_id"], str(artist["name"], 'utf-8'), artist["musicbrainz_artistid"],
                      artist["has_art"]))
            except TypeError:
                c.execute("""\
                    insert into lalist(local_id, name, musicbrainz_artistid, fanarttv_has_art) values (?, ?, ?, ?)
                """, (artist["local_id"], cu.get_str(artist["name"]), artist["musicbrainz_artistid"],
                      artist["has_art"]))
            except Exception as e:
                log(e, xbmc.LOGDEBUG)
                traceback.print_exc()
...
def retrieve_distinct_album_artists():
    ...
    db = c.fetchall()
    for item in db:
        artist = {"name": cu.get_str(item[0]), "musicbrainz_artistid": cu.get_str(item[1])}
...
def check_local_albumartist(album_artist, local_artist_list, background=False):
    ...
    for artist in album_artist:  # match album artist to local artist id
        album_artist_1 = {}
        name = cu.get_str(artist_list_to_string(artist["name"]))
        artist_count += 1
        id_ = None
        for local in local_artist_list:
            ...
            if dialog_msg("iscanceled", background=background):
                break
            if name == cu.get_str(artist_list_to_string(local["artist"])):
                id_ = local["artistid"]
                break
        if id_ is not None:
...
def database_setup(background=False):
    ...
    if not xbmcvfs.exists(os.path.join(__cdam__.path_profile(), "settings.xml")):
        dialog_msg("ok", heading=__lng__(32071), message=__lng__(32072), line2=__lng__(32073),
                   background=background)
    ...
    if not local_album_list:
        dialog_msg("ok", heading=__lng__(32130), message=__lng__(32131), background=background)
        log("XBMC Music Library does not exist, aborting database creation")
        return album_count, artist_count, cdart_existing
    dialog_msg("create", heading=__lng__(32021), message=__lng__(20186), background=background)
    conn = connect()
    c = conn.cursor()
    c.execute("""\
    ...
    if dialog_msg("iscanceled", background=background):
        dialog_msg("close", background=background)
        dialog_msg("ok", heading=__lng__(32050), message=__lng__(32051), line2=__lng__(32052),
...
def get_local_albums_db(artist_name, background=False):
    ...
    try:
        if artist_name == "all artists":
            dialog_msg("create", heading=__lng__(32102), message=__lng__(20186), background=background)
            c.execute("""\
        ...
        c.close()
        for item in db:
            album = {"local_id": (item[0]), "title": cu.get_str(item[1]), "artist": cu.get_str(item[2]),
                     "path": cu.get_str(item[3]),
                     "cdart": ast.literal_eval(cu.get_str(item[4])),
                     "cover": ast.literal_eval(cu.get_str(item[5])), "disc": (item[6]),
                     "musicbrainz_albumid": cu.get_str(item[7]), "musicbrainz_artistid": cu.get_str(item[8])}
            # print album
...
def get_local_artists_db(mode="album_artists"):
    ...
    try:
        ...
        c.close()
        for item in db:
            artists = {"local_id": (item[0]), "name": cu.get_str(item[1]),
                       "musicbrainz_artistid": cu.get_str(item[2])}
            if not item[3]:
...
def store_local_artist_table(artist_list, background=False):
    ...
    conn = connect()
    c = conn.cursor()
    dialog_msg("create", heading=__lng__(32124), message=__lng__(20186), background=background)
    c.execute("""\
    ...
    for artist in artist_list:
        ...
        dialog_msg("update", percent=percent, message=__lng__(32124),
                   line2="%s%s" % (__lng__(32125), artist["local_id"]),
                   line3="%s%s" % (__lng__(32028), cu.get_str(artist["name"])), background=background)
        try:
            c.execute("""\
            ...
            c.execute("""\
                insert into local_artists(local_id, name, musicbrainz_artistid, fanarttv_has_art) values (?, ?, ?, ?)
            """, (artist["local_id"], cu.get_str(artist["name"]), artist["musicbrainz_artistid"], "False"))
            count += 1
        except Exception as e:
            log(e, xbmc.LOGDEBUG)
            traceback.print_exc()
    ...
    try:
        for local_artist in local_artist_list:
            ...
            dialog_msg("update", percent=percent, message=__lng__(20186),
                    ...
                    __lng__(32137), cu.get_str(artist_list_to_string(local_artist["artist"]))),
                       background=background)
            count += 1
            for album_artist in local_album_artist_list:
                ...
                if local_artist["artistid"] == album_artist["local_id"]:
                    artist["name"] = cu.get_str(artist_list_to_string(local_artist["artist"]))
            ...
            if not artist:
                try:
                    ...
                    _, artist["musicbrainz_artistid"], _ = get_musicbrainz_artist_id(
                        cu.get_str(artist_list_to_string(local_artist["artist"])))
                except Exception as e:
                    log(e.message)
                    artist["name"] = cu.get_str(artist_list_to_string(local_artist["artist"]))
                    _, artist["musicbrainz_artistid"], _ = get_musicbrainz_artist_id(
...
def refresh_db(background=False):
    ...
    if xbmcvfs.exists(__cdam__.file_addon_db()):
        ...
        if not background:
            db_delete = dialog_msg("yesno", message=__lng__(32042), background=background)
        else:
            db_delete = True
        if db_delete:
            if xbmcvfs.exists(__cdam__.file_addon_db()):
...
def check_album_mbid(albums, background=False):
    ...
    for album in albums:
        ...
        if dialog_msg("iscanceled", background=background):
            canceled = True
            break
        dialog_msg("update", percent=percent, message=__lng__(32150),
                   line2="%s: %s" % (__lng__(32138), cu.get_str(album["title"])),
                   line3="%s: %s" % (__lng__(32137), cu.get_str(album["artist"])), background=background)
...
def check_artist_mbid(artists, background=False, mode="all_artists"):
    ...
    for artist in artists:
        ...
        if update_artist["musicbrainz_artistid"]:
            dialog_msg("update", percent=percent, message=__lng__(32149),
    ...
    for album in albums:
        ...
        if (len(album["musicbrainz_albumid"]) != 36 and not repair) or (
            ...
            if dialog_msg("iscanceled", background=background):
                canceled = True
                break
            dialog_msg("update", percent=percent, message=__lng__(32133),
                       background=background)
            musicbrainz_albuminfo, _ = get_musicbrainz_album(cu.get_str(album["title"]),
                                                             cu.get_str(album["artist"]), 0, 1)
...
def update_database(background=False):
    ...
    dialog_msg("create", heading=__lng__(32134), message=__lng__(32105),
    ...
    for item in album_detail_list:
        album_detail_list_indexed[(
            item["disc"], cu.get_str(item["artist"]), cu.get_str(item["title"]), item["cover"], item["cdart"],
            item["local_id"], cu.get_str(item["path"]))] = item
    for item in local_album_list:
        if (item["disc"], cu.get_str(item["artist"]), cu.get_str(item["title"]), item["cover"], item["cdart"],
                item["local_id"], cu.get_str(item["path"])) in album_detail_list_indexed:
            matched.append(item)
    for item in matched:
        matched_indexed[(
            item["disc"], cu.get_str(item["artist"]), cu.get_str(item["title"]), item["cover"], item["cdart"],
            item["local_id"], cu.get_str(item["path"]))] = item
    for item in album_detail_list:
        if not (item["disc"], cu.get_str(item["artist"]), cu.get_str(item["title"]), item["cover"],
                item["cdart"], item["local_id"], cu.get_str(item["path"])) in matched_indexed:
    ...
    if __cfg__.enable_all_artists():
        ...
        for artist in local_artists:
            new_artist = {"name": cu.get_str(artist_list_to_string(artist["artist"])),
        ...
        for item in local_artists:
            local_artists_indexed[(item["local_id"], cu.get_str(item["name"]))] = item
        for item in local_artists_db:
            if (item["local_id"], cu.get_str(item["name"])) in local_artists_indexed:
                local_artists_matched.append(item)
        for item in local_artists_matched:
            local_artists_matched_indexed[(item["local_id"], cu.get_str(item["name"]))] = item
        for item in local_artists:
            if not (item["local_id"], cu.get_str(item["name"])) in local_artists_matched_indexed:
...
def backup_database():
    ...
    addon_backup_path = sanitize(os.path.join(__cdam__.path_profile(), db_backup_file))
...
def unset_cdart(path):
    conn = connect()
    c = conn.cursor()
    c.execute("""\
        UPDATE alblist SET cdart=? WHERE path=?
    """, (False, path))
    conn.commit()
    c.close()
